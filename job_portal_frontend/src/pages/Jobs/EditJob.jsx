...
import { singleJob } from "../SingleJob/SingleJobActions";
...
const EditJob = () => {
  ...
  const onFormSubmit = (data) => {
    ...
    dispatch(updateJobAction(data, id, navigate));
  };
  ...
  useEffect(() => {
    if (job && Object.keys(job)?.length > 0 && job.id) {
      reset({
        title: job.title,
